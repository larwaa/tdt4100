...
public class Folder {
	...
	public boolean contains(Folder folder) {
		if (folder == this) { // Seg selv, returner sann
			return true;
		} else if (folder == null) {
			return false;
		} else {
			return this.contains(folder.parent); // sjekke parent
		}
	}
	...
	public Object findFirst(String pattern) {
		...
		// Sjekke subfoldere for innhold
		for (Folder folder : folders) {
			
			// Object siden dette kan gjelde File og Folder
	...
	public List<File> findAllFiles(Predicate<File> pred) {
		// oppretter en liste, og kaller en ny metode som sender med denne
	...
	private void findAllFiles(Predicate<File> pred, List<File> result) {
		for (File file : files) {
			// Sjekker predikattesten for alle filer i denne folderen
			if (pred.test(file)) {
				result.add(file); // Endelig funnet en match
			}
		}
		for (Folder folder : folders) {
			// Kaller samme for alle subfoldere, rekursivt
